	...
	light(shader, depthShader)
{
	...
	roomIdLoc = glGetUniformLocation(shader.GetShaderProgram(), "roomId");
...
void Graphics::Draw(gps::Model3D model, glm::vec3 position, glm::vec3 rotation, glm::vec3 scale, GLint roomId)
{
	glUniform1i(roomIdLoc, roomId);
...
void Graphics::AddLightSource(glm::vec3 position, glm::vec3 target, glm::vec3 color, GLint id)
{
	light.AddLightSource(position, target, color, id);
	light.SendToShader();
}
